...
def training_start():
    ...
    def breath_label_traning_select_tank():
        off_input()
        add_display(breathing_label(0, 0, settings.width, settings.height, (0, 0, 0), 0, delay=0.01, for_from=0, for_before=255, tick=5, function=start_traning))
    ...
    add_display(image_button(settings.width - (settings.height/120 * 48), settings.height/10, 'buttons/button_clear_left.png', scale=settings.height/120, center=False, function=breath_label_traning_select_tank, image_selected='buttons/button_clear_left_selected.png', text='start', text_indent= settings.height//25, shadow=graphics_settings.shadows_buttons))
    add_display(head_menu(align_top=False))
